...
class Player{
    ...
    draw(){
        ctx.beginPath();
        //ctx.arc(this.position.x, this.position.y, this.radius, 0.75, Math.PI * 2-0.75)
        ctx.drawImage(image1,this.position.x-15, this.position.y-15,35,35);
...
function animate(){
    ...
    //detector de colisiones entre fantasma y jugador
    for(let i=ghosts.length-1; 0<=i;i--){
        const ghost = ghosts[i];
    //si el jugador toca el fantasma
      if(
        Math.hypot(
    ...
    for(let i=powerUps.length-1; 0<=i;i--){
        ...
        //jugador colisiona con un powerUp
        if(
            Math.hypot(
    ...
    for(let i=pellets.length-1; 0<=i;i--){
        ...
        if(
            ...
            pellets.splice(i,1)
            score += 10; //puntuacion por cada pellet consumido
